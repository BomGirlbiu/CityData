...
def generate_province_city_dict(cities):
    result = {}
    for city in cities:
        city_key = None
        for key, value in city_dict.items():
            if value == city:
                ...
                break
        if city_key:
            for province, city_list in province_city_dict.items():
                if city_key in city_list:
                    if province not in result:
                        result[province] = []
                    result[province].append(city_key)
                    break
        else:
            print(f"城市 {city} 未找到对应的键")
    return result

if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("用法: python generate_pcd.py <城市中文名1> <城市中文名2> ...")
        sys.exit(1)

    input_cities = sys.argv[1:]
    result_dict = generate_province_city_dict(input_cities)

    with open('province_city_dict.json', 'w', encoding='utf-8') as f:
        json.dump(result_dict, f, ensure_ascii=False, indent=4)

    print("生成 province_city_dict.json 文件成功")